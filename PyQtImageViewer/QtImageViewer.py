...
class QtImageViewer(QGraphicsView):
    ...
    Use the setImage(im) method to set the image data in the viewer.
        - im can be a QImage, QPixmap, or NumPy 2D array.
    ...
    def setImage(self, image, swap_rgb=True):
        """ Set the scene's current image pixmap to the input QImage or QPixmap.
        Raises a RuntimeError if the input image has type other than QImage or QPixmap.
        :type image: QImage | QPixmap | np.ndarray
        """
        if type(image) is QPixmap:
            ...
            if swap_rgb:  # 如果需要，可以进行颜色通道的交换（例如，将BGR转换为RGB）
